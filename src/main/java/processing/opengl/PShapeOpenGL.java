...
//  public int glPointIndex;
  ...
  public PShapeOpenGL(PGraphicsOpenGL pg, int family) {
    ...
    if (family == PShape.GEOMETRY || family == PShape.PRIMITIVE || family == PShape.PATH) {
  ...
  public static PShapeOpenGL createShape(PGraphicsOpenGL pg, PShape src) {
    ...
    if (src.getFamily() == PConstants.GROUP) {
      //dest = PGraphics3D.createShapeImpl(pg, GROUP);
      dest = (PShapeOpenGL) pg.createShapeFamily(PConstants.GROUP);
      copyGroup(pg, src, dest); }
//    } else if (src.getFamily() == PShape.PRIMITIVE) {
...
//    pg.curveTightness(tightness);
  ...
  public int getVertexCount() {
    if (family == PConstants.GROUP) return 0; // Group shapes don't have vertices
    else {
      if (family == PShape.PRIMITIVE || family == PShape.PATH) {
  ...
  public int getFill(int index) {
    if (family != PConstants.GROUP && image == null) {
  ...
  public int[] getVertexCodes() {
    if (family == PConstants.GROUP) return null;
    else {
      if (family == PShape.PRIMITIVE || family == PShape.PATH) {
        // the input geometry of primitive and path shapes is built during
        // tessellation
  ...
  public int getVertexCodeCount() {
    if (family == PConstants.GROUP) return 0;
    else {
      if (family == PShape.PRIMITIVE || family == PShape.PATH) {
        // the input geometry of primitive and path shapes is built during
        // tessellation
      ...
      if (vertices[0].length == 2) {  // tessellating a 2D path
        ...
        for (int j = 0; j < vertexCodeCount; j++) {
          ...
          case PConstants.QUADRATIC_VERTEX:
            inGeo.addQuadraticVertex(vertices[idx+0][PConstants.X], vertices[idx+0][PConstants.Y], 0,
                                     vertices[idx+1][PConstants.X], vertices[idx+1][PConstants.Y], 0,
                                     brk);
            brk = false;
            idx += 2;
            break;
          ...
          case PConstants.BEZIER_VERTEX:
            inGeo.addBezierVertex(vertices[idx+0][PConstants.X], vertices[idx+0][PConstants.Y], 0,
                                  vertices[idx+1][PConstants.X], vertices[idx+1][PConstants.Y], 0,
                                  vertices[idx+2][PConstants.X], vertices[idx+2][PConstants.Y], 0,
                                  brk);
            brk = false;
            idx += 3;
            break;
          ...
          case PConstants.CURVE_VERTEX:
            inGeo.addCurveVertex(vertices[idx][PConstants.X], vertices[idx][PConstants.Y], 0, brk);
            brk = false;
            idx++;
            break;
