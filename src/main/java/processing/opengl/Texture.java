...
public class Texture implements PConstants {
  ...
  public void set(int[] pixels) {
    set(pixels, 0, 0, width, height, PConstants.ARGB);
  }
  ...
  public void set(int[] pixels, int x, int y, int w, int h) {
    set(pixels, x, y, w, h, PConstants.ARGB);
  }
  ...
  protected void updateTexelsImpl(int x, int y, int w, int h) {
    ...
    if (!modified) {
      mx1 = Math.max(0, x);
      mx2 = Math.min(width - 1, x2);
      my1 = Math.max(0, y);
      my2 = Math.min(height - 1, y2);
      modified = true;

    } else {
      if (x < mx1) mx1 = Math.max(0, x);
      if (x > mx2) mx2 = Math.min(width - 1, x);
      if (y < my1) my1 = Math.max(0, y);
      if (y > my2) my2 = y;

      if (x2 < mx1) mx1 = Math.max(0, x2);
      if (x2 > mx2) mx2 = Math.min(width - 1, x2);
      if (y2 < my1) my1 = Math.max(0, y2);
      if (y2 > my2) my2 = Math.min(height - 1, y2);
    }
  }
...
//    }
  ...
  protected void setParameters(Parameters params) {
    ...
    if (params.format == PConstants.RGB)  {
      glFormat = PGL.RGB;
    ...
    if (params.wrapU == PConstants.CLAMP) {
      glWrapS = PGL.CLAMP_TO_EDGE;
    ...
    if (params.wrapV == PConstants.CLAMP) {
      glWrapT = PGL.CLAMP_TO_EDGE;
  ...
  static public class Parameters {
    ...
    public Parameters() {
      ...
      this.mipmaps = true;
      this.wrapU = PConstants.CLAMP;
      this.wrapV = PConstants.CLAMP;
    }
