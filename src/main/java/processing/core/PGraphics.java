 ...
 * <ul>
 * <li>setPApplet(PApplet) - is called to specify the PApplet PApplet.
...
public void setPrimary(boolean primary) {  // ignore
  ...
  // headache reasons.. argh, a semi-transPApplet opengl surface?)
  // use createGraphics() if you want a transPApplet surface.
  if (primaryGraphics) {
  //  format = RGB;
  }
}
 ...
 * out so that setPApplet/setPrimary/setPath can be handled differently.
 *
...
/**
 * Start and stop are in radians, converted by the PApplet function.
...
public PFont createFont(String name, float size,
  ...
  try {
    ...
    return new PFont(baseFont.deriveFont(size * this.pixelDensity),
...
//      }
    ...
    if (textFontNative != null) {
      ...
      // For instance, PApplet.getGraphics() doesn't work for OpenGL since
      // an OpenGL drawing surface is an embedded component.
//        if (PApplet != null) {
//          textFontNativeMetrics = PApplet.getGraphics().getFontMetrics(textFontNative);
//        }
 ...
 * specified colors or made transPApplet by setting the alpha.<br />
 * <br />
 * To make an image transPApplet, but not change it's color, use white as
 * the tint color and specify an alpha value. For instance, tint(255, 128)
 * will make an image 50% transPApplet (unless <b>colorMode()</b> has been
 * used).<br />
 * <br />
 * When using hexadecimal notation to specify a color, use "#" or "0x"
 * before the values (e.g. #CCFFAA, 0xFFCCFFAA). The # syntax uses six
 ...
 * drawing surface cannot be set transPApplet.</p>
 ...
 * @param method The method name (no PAppletheses)
 */
